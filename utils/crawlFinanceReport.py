...
if __name__ == '__main__':
    ...
    for i, row in df.iterrows():
        reportCode = row.iloc[2]
        link_pdf = get_report_pdf_link(reportCode)
        if link_pdf:
            download_pdf(reportCode, link_pdf, pdf_path)
        else:
            print(f"Không tìm thấy link pdf của {reportCode}")
            noReportPDF.append(row)
            continue

    noReportPDF_df = pd.DataFrame(noReportPDF, columns=["id_group", "company_name", "stock_code"])
    noReportPDF_filename = "NhomKhongCoBCTC2024.csv"
